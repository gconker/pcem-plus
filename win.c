...
int video_shaders_index = 0;
char video_shaders_name[100];
...
//        Sleep(500);
        ...
        while (!quited)
        {
                ...
                if (leave_fullscreen_flag && video_fullscreen)
                {
                        leave_fullscreen_flag = 0;
                        SendMessage(ghwnd, WM_LEAVEFULLSCREEN, 0, 0);
                }
                else if (enter_fullscreen_flag && !video_fullscreen)
                {
                        enter_fullscreen_flag = 0;
                        SendMessage(ghwnd, WM_ENTERFULLSCREEN, 0, 0);
                }
        }
        mainthreadon=0;
}
...
//                        pclog("WM_COMMAND %i\n",LOWORD(wParam));
                ...
                switch (LOWORD(wParam))
                {
                        ...
                        if (video_fullscreen_first)
                        {
                                ...
                                MessageBox(hwnd, "Use ALT + ENTER to switch between windowed and fullscreen modes", "PCem", MB_OK);
                        }
                        draw_fullscreen();
                        break;
                        ...
                        if (LOWORD(wParam)>=IDM_VID_SHADERS_NONE && LOWORD(wParam)<(IDM_VID_SHADERS_NONE+100))
                        {
                                ...
                                CheckMenuItem(hmenu, IDM_VID_SHADERS_NONE + video_shaders_index, MF_UNCHECKED);
                                // new index
                                video_shaders_index = LOWORD(wParam) - IDM_VID_SHADERS_NONE;
                                //check selected and set video_shaders_name variable
                                CheckMenuItem(hmenu, IDM_VID_SHADERS_NONE + video_shaders_index, MF_CHECKED);
                                GetMenuString(hmenu, IDM_VID_SHADERS_NONE + video_shaders_index, video_shaders_name, 80, 0);
                                saveconfig();

                                // Close and init device with new shader
                                startblit();
                                vid_apis[0][vid_api].close();
                                vid_apis[0][vid_api].init(ghwnd);
                                endblit();
